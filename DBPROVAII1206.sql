...
/* 1 */
SELECT alunos.nome, alunos.idade FROM alunos WHERE curso IN ('Informatica');

/* 2 */
SELECT * FROM professores WHERE disciplina IN ('Administração');
...
/* 4 */
SELECT COUNT(*) FROM alunos WHERE curso IN ('Contabilidade', 'Informatica', 'Administração')
GROUP BY curso;

/* 5 
SELECT alunos.curso FROM alunos;
*/

/* 6 */
SELECT alunos.nome AS Alunos_Turma_A FROM alunos
...
/* 9 */
SELECT COUNT(*) FROM matriculas WHERE matriculas.id_turma = 'Turma B';

/* 10

SELECT professores.nome FROM professores
INNER JOIN turmas ON turmas.nome_turma = turma.id_professor;
